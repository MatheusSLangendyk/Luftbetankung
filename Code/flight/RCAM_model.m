...
u1 = U(1); %d_A (alieron 副翼) 
u2 = U(2); %d_T (stabilizer 稳定器)
u3 = U(3); %d_R (rudder 舵)
u4 = U(4); %d_th1 (throttle 1 风门1)
u5 = U(5); %d_th2 (throttle 2 风门2)

%--------------------CONSTANTS-------------------------
%Nominal vehicle constants 
m = 120000;            %Aircraft total mass (kg)
%NOTE: We will define Ib and invib later 英磅

cbar = 6.6;            %Mean Aerodynamic Chord (m) 平均空气动力学弦：对于任意平面形状的实际机翼，它的弦长从翼根到翼尖是变化的。可以假想存在一个相当的矩形机翼，此矩形机翼与实际机翼的面积相同，俯仰力矩和气动力合力也相同。我们把这样的矩形机翼的弦称为机翼的平均空气动力弦。
lt = 24.8;             %Distance by AC of tail and body(m)
S = 260;               %Wing planform area (m^2) 机翼平面区域
St = 64;               %Tail planform area（m^2）

Xcg = 0.23*cbar;       %x position of CoG in Fm (m)  CoG:(Center of gravity of an aircraft)
...
alpha_switch = 14.5*(pi/180); %alpha where lift slope goes from linear to non-limit
...
%----------------3. AERODYNAMIC FORCE COEFFIFIENTS--------------------
%Calculate the CL_wb
if alpha<=alpha_switch
    CL_wb = n*(alpha -alpha_L0);
else
    CL_wb = a3*alpha^3 + a2*alpha +a0;
end
...
%Total drag force (neglecting tail 忽略尾巴)
